...
class GaussianModel:
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        fused_color = RGB2SH(torch.tensor(np.asarray(pcd.colors)).float().cuda()) #TODO RGB转到球谐系数
        features = torch.zeros((fused_color.shape[0], 3, (self.max_sh_degree + 1) ** 2)).float().cuda()
        features[:, :3, 0 ] = fused_color #每个点的每种颜色有16个feat(球谐系数)，第0个初始化为颜色的球谐值
        features[:, 3:, 1:] = 0.0
        ...
        opacities = inverse_sigmoid(0.1 * torch.ones((fused_point_cloud.shape[0], 1), dtype=torch.float, device="cuda")) #TODO 不透明度

        self._xyz = nn.Parameter(fused_point_cloud.requires_grad_(True)) #(point,3)
        self._features_dc = nn.Parameter(features[:,:,0:1].transpose(1, 2).contiguous().requires_grad_(True)) #(point,1,3)
        self._features_rest = nn.Parameter(features[:,:,1:].transpose(1, 2).contiguous().requires_grad_(True)) #(point,15,3)
        self._scaling = nn.Parameter(scales.requires_grad_(True)) #(point,3)
        self._rotation = nn.Parameter(rots.requires_grad_(True)) #(point,4) _scaling和_rotation组成3d高斯的协方差矩阵
        self._opacity = nn.Parameter(opacities.requires_grad_(True)) #(point,1)
